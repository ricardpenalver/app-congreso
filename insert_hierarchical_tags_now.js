// Script directo para insertar etiquetas jerarquizadas (sin esperar confirmación)
...
async function run() {
    ...
    console.log('\n📝 Insertando etiquetas jerarquizadas...\n');
...
async function insertHierarchicalTags() {
    ...
    console.log(`Total de registros: ${allTags.length}`);
    ...
    for (let i = 0; i < allTags.length; i += BATCH_SIZE) {
        ...
        if (success) {
            ...
            console.log(`✓ Lote ${batchNumber}/${totalBatches} - Insertadas: ${insertedCount}/${allTags.length}`);
        } else {
            errorCount++;
            console.log(`✗ Error en lote ${batchNumber} (puede ser restricción UNIQUE)`);
        }

        await new Promise(resolve => setTimeout(resolve, 200));
    }

    console.log(`\n${errorCount > 0 ? '⚠️' : '✅'} Completado: ${insertedCount} registros insertados`);

    if (errorCount > 0) {
        console.log('\n⚠️  Algunos lotes fallaron. Si ves errores 409 (Conflict), necesitas ejecutar:');
    ...
    tagsData.slice(0, 10).forEach((tag, i) => {
        const bar = '█'.repeat(Math.floor(tag.count / 10));
        console.log(`${String(i + 1).padStart(2)}. ${tag.name.padEnd(30)} ${String(tag.count).padStart(3)} ${bar}`);
    });
}
